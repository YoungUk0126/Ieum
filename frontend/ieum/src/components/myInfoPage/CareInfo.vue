...
onMounted(() => {
  ...
  getCareInfo((response) => {
    if (response.data.data.careSerial === null) {
      swal({
        title: '알림',
        text: '피보호자 정보가 등록되지 않았습니다.',
        icon: 'info',
        buttons: {
          confirm: {
            text: '확인',
        ...
        router.push({ name: 'TheCareInfoModifyView' })
      })
    }
...
<template>
  <div class="careInfo" v-if="careInfo.careName">
    ...
    <div class="info">
      <div class="nameInfo">
        <div class="text-xs">{{ careInfo.careName }} &nbsp;</div>
        <div v-if="careInfo.careGender === 'FEMALE'" class="text-xs">할머니</div>
        <div v-else class="text-xs">할아버지</div>
      </div>
  ...
  <div v-else class="noInfo">등록된 보호자 정보가 없습니다</div>
