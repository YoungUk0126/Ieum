...
//            Enum 자동 매핑 되는지 확인할 것
    ...
    public void deletePillInfo(Long pillInfoNo) throws Exception {
        try {
            PillInfo pillInfo = pillInfoRepository.findById(pillInfoNo)
                    .orElseThrow(() -> new NoSuchElementException("NOT FOUND PILL INFO"));

            pillTimeRepository.deleteAllByPillInfo_PillInfoNo(pillInfoNo);
            pillInfoRepository.deleteById(pillInfoNo);

            // socket으로 약 정보 전송
            socketService.sendPillDataToIot(pillInfo.getCareInfo().getCareNo());
    ...
    public void deletePillTime(Long pillTimeNo) throws Exception {
        try {
            PillTime pillTime = pillTimeRepository.findById(pillTimeNo)
                    .orElseThrow(() -> new NoSuchElementException("NOT FOUND PILL TIME"));
            pillTimeRepository.deleteById(pillTimeNo);

            // socket으로 약 정보 전송
            socketService.sendPillDataToIot(pillTime.getPillInfo().getCareInfo().getCareNo());
        } catch (EmptyResultDataAccessException e) {
