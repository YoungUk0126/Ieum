...
public class SleepServiceImpl implements SleepService {
    ...
    public SleepResponseDto getSleep() throws Exception {
        try {
            ...
            SleepInfo sleep = sleepRepository.findByCareInfo_CareNo(careNo)
                    .orElseThrow(() -> new NoSuchElementException("존재하지 않는 취침 시간 데이터입니다."));
    ...
    public void deleteSleep(Long sleepInfoNo) throws Exception {
        try {
            SleepInfo sleepInfo = sleepRepository.findById(sleepInfoNo)
                    .orElseThrow(() -> new NoSuchElementException("NOT FOUND SLEEP INFO"));
            sleepRepository.deleteById(sleepInfoNo);

            // socket 전송
            socketService.sendSleepDataToIot(sleepInfo.getCareInfo().getCareNo());
